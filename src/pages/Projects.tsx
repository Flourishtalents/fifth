...
export default function Projects() {
  ...
  const [activeTab, setActiveTab] = useState<'hire' | 'apply' | 'sponsor' | 'recognition'>(
  ...
  const sortByCost = <T,>(items: T[], get: (t: T) => number) => {
    if (sortBy === 'none') return items;
    return items.slice().sort((a, b) => (sortBy === 'price-asc' ? get(a) - get(b) : get(b) - get(a)));
  };

  const sortedTalents = sortByCost(filteredTalents, (t: any) => t.hourlyRate || 0);
  const sortedTeams = sortByCost(filteredTeams, (t: any) => t.startingRate || 0);
  const sortedAgencies = sortByCost(filteredAgencies, (a: any) => a.startingRate || 0);
  const sortedProjects = sortByCost(filteredProjects, (p: any) => parseUGX(p.budget));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-300">Hire, Apply, Sponsor, and celebrate outstanding work.</p>
          </div>
          <div className="flex items-center gap-3">
            <SlidersHorizontal className="text-gray-300 w-5 h-5" />
            <select
              value={sortBy}
            ...
            >
              <option value="none" className="bg-gray-800">Sort by cost</option>
              <option value="price-asc" className="bg-gray-800">Cost: Low → High</option>
              <option value="price-desc" className="bg-gray-800">Cost: High → Low</option>
            </select>
          </div>
        </div>

        {/* Top Tabs: Hire | Apply | Sponsor | Recognition */}
        <div className="flex space-x-1 glass-effect p-2 rounded-xl overflow-x-auto whitespace-nowrap md:w-full mb-6">
          <button
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 items-stretch md:items-center">
          ...
          <div className="md:basis-1/2 relative">
            ...
            <input
              type="text"
              placeholder={
                activeTab === 'apply' ? 'Search jobs...' : activeTab === 'hire' ? 'Search talent, teams, agencies...' : activeTab === 'sponsor' ? 'Search sponsorships...' : 'Search recognitions...'
              }
              value={searchQuery}
